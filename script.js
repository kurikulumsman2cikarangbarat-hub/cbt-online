...
let exam = { 
    ...
    optionStates: {}, 
    cheatCount: 0,
    tabSwitchCount: 0 // TAMBAHAN: Counter untuk berpindah tab
};
...
function showWelcome() {
    const welcomeText = `
        ...
        <!-- PERINGATAN BARU TENTANG PENCATATAN PELANGGARAN -->
        <div style="background: #fff3cd; padding: 15px; border-radius: 10px; margin-bottom: 20px; border-left: 4px solid #ff9800;">
            <p style="margin: 0 0 10px 0; color: #856404; font-weight: bold;">⚠️ PERINGATAN KEAMANAN:</p>
            <p style="margin: 0; color: #856404; font-size: 0.95rem;">
                Sistem akan <strong>mencatat otomatis</strong> setiap kali Anda meninggalkan halaman ujian 
                (pindah tab, pindah aplikasi, atau minimize browser). 
                <br>Data pelanggaran akan dilaporkan ke pengawas ruang.
            </p>
        </div>
...
async function submitData() {
    ...
    // PERUBAHAN: Logika pengecekan jawaban benar
    sorted.forEach(q => {
    ...
    const payload = {
        ...
        jml_curang: exam.tabSwitchCount, // TAMBAHAN: Kirim jumlah pelanggaran
    ...
    try {
        ...
        // TAMBAHAN: Tampilkan laporan pelanggaran di halaman akhir
...
// TAMBAHAN: Event listener yang ditingkatkan untuk mencatat berpindah tab
document.addEventListener("visibilitychange", () => {
    if (document.hidden && document.getElementById('view-exam').classList.contains('active')) {
        exam.tabSwitchCount++; // Tambah counter pelanggaran
        exam.cheatCount++; // Tetap pertahankan untuk kompatibilitas
        
        // Tampilkan notifikasi dengan jumlah pelanggaran
        showToast(`⚠️ Peringatan: Jangan pindah tab! (Pelanggaran: ${exam.tabSwitchCount}x)`);
        
        // Notifikasi lebih serius jika pelanggaran berulang
        if (exam.tabSwitchCount >= 3) {
            showToast(`🚨 PERINGATAN AKHIR! Anda telah ${exam.tabSwitchCount}x meninggalkan halaman!`);
        }
    }
});
